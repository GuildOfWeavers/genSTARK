...
function buildInputRegisters(inputs, context, domain) {
    const iRegisters = new Array(context.registerCount);
    for (let i = 0; i < inputs.length; i++) {
        iRegisters[i] = new registers_1.InputRegister(inputs[i], context, domain);
    }
    return iRegisters;
}
