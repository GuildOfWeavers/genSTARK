 ...
 * function (https://eprint.iacr.org/2019/426.pdf). In this specific instance, the parameters
 * for the function are chosen as follows:
 * - p (field modulus)      : 2^128 - 9 * 2^32 + 1
 * - m (number of registers): 4
 * - N (rounds)             : 32
 *
 * The modifications as compared to the original construct are:
 * - The second step of the last round is omitted from the computation;
 * - The first step of the first round is pre-computed
 * - The function is restricted to accepting a single value < p, and producing a hash value that
 *   is also < p.
 *
 * All constants were generated by using code from here: https://github.com/KULeuven-COSIC/Marvellous
 */
...
define Rescue2x64 over prime field (2^128 - 9 * 2^32 + 1) {
    ...
    alpha: 3;
    invAlpha: 0-113427455640312821154458202464371168597;
