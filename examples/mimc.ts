...
import { ExecutionFrame, EvaluationFrame, FiniteField, ConstantPattern } from '@guildofweavers/genstark';
...
function mimcTransition(frame: ExecutionFrame, field: FiniteField) {
...
function mimcConstraint(frame: EvaluationFrame, field: FiniteField) {
    const v = frame.getValue(0);
    const k = frame.getConst(0);
